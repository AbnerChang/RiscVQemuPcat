...
void riscv_cpu_do_interrupt(CPUState *cs)
{
 ...
 /*   if (cs->exception_index & (0x1 << 31)) {
    ...
    if (riscv_cpu_get_current_mode (env) == RISCV_SUPERVISOR_MODE_PRIVILEGE) {
        //printf("QEMU-RISCV: Do interrupt in  RISCV_SUPERVISOR_MODE_PRIVILEGE\n"); 
        ...
        //
        //printf("QEMU-RISCV: Do interrupt in  RISCV_MACHINE_MODE_PRIVILEGE\n"); 
        // Manage machine privilege stack.
        // push privilege.
        //
        env->helper_csr[CSR_MACHINE_MSTATUS] = (env->helper_csr[CSR_MACHINE_MSTATUS] & ~ CSR_MACHINE_MODE_PRIVILEGE_STACK_MASK) | \
            ((env->helper_csr[CSR_MACHINE_MSTATUS] << CSR_MACHINE_MODE_PRIVILEGE_STACK_PUSH_SHIFT) & CSR_MACHINE_MODE_PRIVILEGE_STACK_MASK);
        //
        //Set current privilege to machine with IE = 0;
        //
        env->helper_csr[CSR_MACHINE_MSTATUS] |= (RISCV_MACHINE_MODE_PRIVILEGE << CSR_MACHINE_MSTATUS_PRV_POS);
        ...
        // FINALLY, set PC to value in evec register and return
        // 
        //printf("QEMU-RISCV: Do machine mode int %lx %lx\n", env->helper_csr[CSR_MACHINE_MSTATUS], env->helper_csr[CSR_MACHINE_MTVEC]); 
        //

        // Save gpr before interrupt.
        riscv_save_gpr (env);
        //riscv_cpu_dump(cs);
        env->active_tc.PC = env->helper_csr[CSR_MACHINE_MTVEC] + 0xc0;
    }
...
{
    return ((env->helper_csr[CSR_MACHINE_MSTATUS] & CSR_MACHINE_MSTATUS_PRV_MASK) >> CSR_MACHINE_MSTATUS_PRV_POS);
}
