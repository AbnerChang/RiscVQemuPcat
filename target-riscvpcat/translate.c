...
inline static int csr_regno(int regno)
{
    ...
    if (((regno & RISCV_PRIVILEGE_MODE_MASK) >> RISCV_PRIVILEGE_MODE_SHIFT) == RISCV_MACHINE_MODE_PRIVILEGE) {
        //
        // This is machine mode CSR.
        //
        return regno;
...
RISCVCPU *cpu_riscv_init(const char *cpu_model)
{
    ...
    env->helper_csr[CSR_MACHINE_MSTATUS] = def->machine_mode_init_status_reg;
...
void riscv_save_gpr (CPURISCVState *env)
{
    ...
    //printf("Risc-v: Save gp registers\n");
    for (index = 0; index < 32; index ++) {
        env->saved_active_tc.gpr[index] = env->active_tc.gpr [index];
    }
}
...
void riscv_restore_gpr (CPURISCVState *env)
{
    ...
    //printf("Risc-v: Restore gp registers\n");
    for (index = 0; index < 32; index ++) {
        env->active_tc.gpr [index] = env->saved_active_tc.gpr[index];
    }
}
