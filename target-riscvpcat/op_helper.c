...
inline void csr_write_helper(CPURISCVState *env, target_ulong val_to_write, target_ulong csrno)
{
    ...
    switch (csrno) {
        ...
        case CSR_CYCLE:
            ...
            break;
        case CSR_MACHINE_MTIME:
            cpu_riscv_machine_store_count(env, (uint32_t)val_to_write, 0);
            break;
        case CSR_MACHINE_MTIMECMP:
            cpu_riscv_machine_store_compare(env, (uint32_t)val_to_write);
            break;
        case CSR_FCSR:
...
inline target_ulong csr_read_helper(CPURISCVState *env, target_ulong csrno)
{
    switch (csrno) {
        ...
        case CSR_CYCLE:
            ...
            break;
        case CSR_MACHINE_MTIME:
            return cpu_riscv_machine_get_count(env);
            break;
        case CSR_FCSR:
...
target_ulong helper_sret(CPURISCVState *env)
{
    ...
    // finally, return EPC value to set cpu_PC
    //printf("RISC-V:Machine ERET\n");
    if (env->MachineERetCallback != NULL) {
        env->MachineERetCallback (env);
    }
    // Restore gpr.
    riscv_restore_gpr (env);
    //riscv_cpu_dump (NULL);
    return env->helper_csr[CSR_EPC]; 
}
